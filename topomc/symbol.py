...
class AreaSymbol(Symbol):
    ...
    def plot(self, area):
        plt.fill(*Coordinates.to_list(area), facecolor=self.fillcolor, edgecolor=self.bordercolor, linewidth=self.borderwidth)
...
class PointSymbol(Symbol):
    def set_properties(self, color, pointsize, icon=None):
        ...
        if icon:
            ...
            doc = minidom.parse(path.join(path.dirname(__file__), "assets", "symbols", f"{self.__class__.__name__}.svg"))
            icon = parse_path(
                [path.getAttribute('d') for path in doc.getElementsByTagName('path')][0]
            )
            doc.unlink()
