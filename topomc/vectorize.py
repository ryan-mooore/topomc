...
def vectorize(heightmap):
    ...
    def pixline_tracer(first_iter=False, isoline=Isoline(), this_cell=None, this_cell_link=None):
        ...
        for this_pixline in this_cell.pixlines:
            if this_pixline.height == height:
                ...
                this_heightplane.bitmap[this_cell.coords.y][this_cell.coords.x] = 1
                ...
                elif opposite_link.x == 1: cell_link_buider(( 1, 0), (0, '~')) # right
                elif opposite_link.y == 0: cell_link_buider(( 0, 1), ('~', 1))  # bottom
                elif opposite_link.y == 1: cell_link_buider(( 0,-1), ('~', 0)) # top
                else:
    ...
    this_topodata = Topodata()

    for height in range(*chunk_tile.get_extremes()):
        this_heightplane = Heightplane(height)
        this_heightplane.bitmap = [[0 for  cell_row in chunk_tile.cells] for cell in chunk_tile.cells[0]]
        
        for y, row in enumerate(chunk_tile.cells):
            for x, cell in enumerate(row): # foreach cell
                if this_heightplane.bitmap[y][x] is not 1: #if it has not already been used at this height
                    ...
                    this_heightplane.isolines.append(
                        pixline_tracer(
                            first_iter=True,
        ...
        this_topodata.heightplanes.append(this_heightplane)
    return this_topodata
