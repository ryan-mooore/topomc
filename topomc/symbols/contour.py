...
class Contour(LinearSymbol):
    settings = ["interval", "smoothness", "index"]
    ...
    def render(self, interval=1, smoothness=1, index=5):
    ...
    def debug(self):
        for value, isoline in enumerate(
            self.topomap.closed_isolines + self.topomap.open_isolines
        ):
            ...
            if isinstance(isoline, ClosedIsoline):
                ...
                )  # add red point at end for OpenIsoline
            ...
            for point1, point2 in zip(isoline.vertices, isoline.vertices[1:]):
                ...
                if isinstance(isoline, OpenIsoline):
                    ...
                    isoline, Hill
                ):  # all ClosedIsolines are anti-clockwise, so hills have downslope on right
                    normal_ang += math.pi
