...
from nbt.nbt import NBTFile, TAG_Compound  # type: ignore
...
class ChunkTile:
    def __init__(
        ...
        chunk_x: int,
        chunk_z: int,
        surface_blocks: list[str] = DEFAULT_SURFACE,
    ) -> None:
        def get_chunktag_heightmap(
            anvil: TAG_Compound,
            version_tag: NBTFile,
            tag: str = "MOTION_BLOCKING_NO_LEAVES",
        ) -> list[list[int]]:
            try:
                tags.index(tag)
            except Exception:
            ...
            chunktag_heightmap_deepened: list[list] = []
            row: list[int] = []
...
class BlockMap:
    def __init__(
        ...
        def horizontal_append(
            map1: list[list[int]], map2: list[list[int]]
        ) -> list[list[int]]:
        ...
        for z in range(chunk_z1, chunk_z2 + 1):
            if chunk_z2 - chunk_z1 > 8:
                Logger.log(
                    logging.info,
                    f"imported {z - chunk_z1 + 1} of {chunk_z2-chunk_z1 + 1}",
                )
            chunk_row: list[list[int]] = []
            chunk_tile_row = []
